...
def convert_prose(t: str) -> str:
    ...
    t = re.sub(r"\\input\{[^}]*\}\n?", "", t)
    t = re.sub(r"\\label\{[^}]*\}\n?", "", t)
    # boxes -> bold-led paragraphs
    t = re.sub(r"\\begin\{decoded\}", r"\\paragraph{Decoded.}", t)
    t = re.sub(r"\\end\{decoded\}", "", t)
    t = re.sub(r"\\begin\{faithnote\}",
               r"\\paragraph{Why this is the right definition.}", t)
    t = re.sub(r"\\end\{faithnote\}", "", t)
    t = re.sub(r"\\begin\{trustbox\}", r"\\paragraph{Trust.}", t)
    t = re.sub(r"\\end\{trustbox\}", "", t)
    t = t.replace("\\noindent\\textbf{Trust.}", "")
    t = t.replace("\\noindent", "")
    return t.strip()
